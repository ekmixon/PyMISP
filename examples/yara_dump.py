    ...
    try:
        ...
        if 'duplicated identifier' in e.args[0]:
            ...
            for item in duplicate_rule_names:
                all_yara_rules = all_yara_rules.replace(
                    f'rule {item}', f'rule duplicate_{item}', 1
                )
            ...
            exit(f"ERROR SyntaxError in rules: {e.args}")
    break
...
print("")
print(
    f"MISP attributes with YARA rules: total={attr_cnt} valid={attr_cnt - attr_cnt_invalid} invalid={attr_cnt_invalid} duplicate={attr_cnt_duplicate} changed={attr_cnt_changed}."
)

print(
    f"Valid YARA rule file save to file '{fname}'. Invalid rules/attributes were ignored."
)
